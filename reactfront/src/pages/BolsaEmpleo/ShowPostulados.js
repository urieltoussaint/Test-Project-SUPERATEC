   ...
   const handleFilterChange = (e) => {
    ...
    const getAllDatos = async (page = 1) => {
        try {
            ...
            const response = await axios.get(`${endpoint}/postulados-estadisticas`, {
    ...
    const deleteDatos = async () => {
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`${endpoint}/participantes-bolsa/${selectedId}`, {
                headers: {
...
const participantesPorEstadoData = statistics?.participantesPorEstado
    ...
    const renderItem = (dato) => (
        <tr key={dato.id}>
            <td className="col-cedulas">{dato?.datos_identificacion?.cedula_identidad}</td>
            <td className="col-nombress">{dato?.datos_identificacion?.nombres}</td>
            <td className="col-apellidoss">{dato?.datos_identificacion?.apellidos}</td>
            ...
            <td className="col-accioness">
                ...
                <Button
                    ...
                    onClick={() => navigate(`/postulados-bolsa-empleo/${dato.id}`)}
                    className="me-2"
                    title="Ver más"
                >
        ...
        <div className="container-fluid " style={{ fontSize: '0.85rem' }}>
            ...
            <div className="stat-box d-flex justify-content-between" style={{ maxWidth: '100%' }}> 
                ...
                <div className="stat-card" style={{ padding: '5px', margin: '0 10px', width: '22%' }}>
                    ...
                    <h4 style={{ fontSize: '1.1rem', color:'gray' }}>Total de Postulados</h4>
                </div>
                    ...
                    <div className="card-box" style={{ padding: '10px' }}> {/* Reduce padding de la tabla */}
                        <div className="d-flex justify-content-between align-items-center mb-3">
                            ...
                            />
                                ...
                                <Button variant="btn custom" onClick={() => navigate('/inscribir-part-bolsa')} className="btn-custom" title="Nuevo Postulante">
            ...
            <div className="col-lg-12 d-flex justify-content-between" style={{ gap: '20px' }}>
                ...
                <div className="chart-box " style={{flex: '1 1 31%', maxWidth: '31%', marginRight: '10px'}}>
                <h4 style={{ fontSize: '1.2rem' }}>Porcentaje por Nivel de Instrucción</h4>
                    {nivelInstruccionOptions.length > 0 && (
